...
function hotel_room_meta_box_callback($post)
{
    ...
    $adult = get_post_meta($post->ID, '_hotel_room_adult', true);
    $children = get_post_meta($post->ID, '_hotel_room_children', true);
    $extrabed = get_post_meta($post->ID, '_hotel_room_extrabed', true);
    $main_amenities = get_post_meta($post->ID, '_hotel_room_main_amenities', true);

?>
    <p>
        <label for="hotel_room_area"><strong>Số lượng người lớn:</strong></label><br>
        <input type="text" name="hotel_room_adult" id="hotel_room_adult" value="<?php echo esc_attr($adult); ?>" style="width:100%;" placeholder="VD: 2" />
    </p>
    <p>
        <label for="hotel_room_area"><strong>Số lượng trẻ em:</strong></label><br>
        <input type="text" name="hotel_room_children" id="hotel_room_children" value="<?php echo esc_attr($children); ?>" style="width:100%;" placeholder="VD: 1" />
    </p>
    <p>
        <label for="hotel_room_extrabed">
            <input type="checkbox" name="hotel_room_extrabed" id="hotel_room_extrabed" value="<?php echo esc_attr($children); ?>"
                <?php checked($extrabed, 1); ?> />
            <strong>Có giường phụ</strong>
        </label>
    </p>
    <p>
...
function hotel_save_room_meta_boxes($post_id)
{
    ...
    if (isset($_POST['hotel_room_adult'])) {
        update_post_meta($post_id, '_hotel_room_adult', sanitize_text_field($_POST['hotel_room_adult']));
    }

    if (isset($_POST['hotel_room_children'])) {
        update_post_meta($post_id, '_hotel_room_children', sanitize_text_field($_POST['hotel_room_children']));
    }

    if (isset($_POST['hotel_room_extrabed'])) {
        update_post_meta($post_id, '_hotel_room_extrabed', sanitize_text_field($_POST['hotel_room_extrabed']));
    }
    // Gọi hàm đồng bộ dữ liệu sau khi lưu thành công
    if (function_exists('hotel_sync_get_rooms')) {
        hotel_sync_get_rooms($post_id);
    }
}
...
if (!function_exists('hotel_get_room_details_html')) {
    function hotel_get_room_details_html($post_id)
    {
        ...
        $extrabed = get_post_meta($post_id, '_hotel_room_extrabed', true);
        ...
        $adult = get_post_meta($post_id, '_hotel_room_adult', true);
        $children = get_post_meta($post_id, '_hotel_room_children', true);
        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <p>
                        <?php echo $adult ?> người lớn, <?php echo $children ?> trẻ em <br>
                        <?php echo $area; ?> /
