...
function callApi($endpoint, $method, $data = [])
{
    ...
    // Lấy cấu hình để kiểm tra xem site này có được cho phép gọi API không
    ...
    // Chỉ thực hiện nếu site có cấu hình API
    if (empty($site_conf) || empty($site_conf['enabled']) || empty($site_conf['token'])) {
        error_log('API Call skipped: Site ID ' . $current_blog_id . ' not configured or disabled');
        return null;
    }
        ...
        if ($method === 'GET' || $method === 'DELETE') {
            ...
            $args = [
                ...
                'headers' => [
                    ...
                    'Accept' => 'application/json',
                    'x-hotel-id' => $current_blog_id, // Send hotel ID via header
                ],
                'body' => $json_body,
                'timeout' => 45
            ];
    ...
    if ($http_code >= 400) {
        ...
        if (isset($parsed_response['message'])) {
            ...
            $error_details['message'] = $parsed_response['message'];
        }

        if (isset($parsed_response['errors'])) {
            $error_details['errors'] = $parsed_response['errors'];
            $error_message .= ' - Validation errors: ' . json_encode($parsed_response['errors']);
        }

        if (isset($parsed_response['data'])) {
            $error_details['data'] = $parsed_response['data'];
        }

        error_log('API Error Details: ' . json_encode(['code' => $http_code, 'body' => $response_body]));
        return new WP_Error('api_http_error', $error_message, ['http_code' => $http_code, 'response' => $parsed_response, 'details' => $error_details]);
    }
    return $response;
}
